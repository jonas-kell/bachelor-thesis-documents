...
The job of the block is to exchange information between the $n$ tokens. 
While the job of the processing the information inside a token's embedding (of size $e$) is performed by the \emph{channel mlp}.
...
A \emph{averaging pooling} operator is used in conjunction with a \emph{mlp} to reshape the output to dimension necessary for the task. 
