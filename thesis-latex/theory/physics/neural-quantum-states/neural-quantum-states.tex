...
The function will be written like \ref{eq:parametrized-wavefunction}.
...
In order to compute a helpful representation, some more identities \ref{eq:completeness-of-base} and \ref{eq:wave-function-probability} are necessary \cite{schwablQM}. \ref{eq:completeness-of-base} applies to \emph{complete orthonormal} bases only.
...
In the derivation, the \emph{local estimator} \ref{eq:local-estimator} is introduced. Because the computational base is sparse, only very few elements of the sum over $s'$ in \ref{eq:local-estimator} are non-zero \cite{jVMCPaper}. A local estimator can therefore be evaluated in constant time.
...
The final result in \ref{eq:derivation-local-estimator} has the form of a \emph{probability distribution} over $s$.
...
By randomly sampling a large, but manageable number $N_\mathrm{MC}$ of states $s_{(n)}$ with the \emph{Metropolis algorithm} \cite{quantumMonteCarloSimulationsOfSolids}, the true value can be approximated \ref{eq:mc-local-estimator} \cite{jVMCPaper}.
...
Therefore the energy of a system can be estimated with \ref{eq:energy-optimization}. All the terms can efficiently be computed, using methods from \fullref{sec:theory-numericalsolution}.
...
\begin{enumerate}
    ...
    \item Estimate its energy with \ref{eq:energy-optimization} and MC-sampled states
